...
async def chat(req: ChatRequest, user: models.User = Depends(get_current_user)):
    """
    Proxies a message to Anthropic's API with a scoped system prompt.
    ...
    if not ANTHROPIC_API_KEY:
        raise HTTPException(
            status_code=503,
            detail="The AI guide isn't configured yet (missing ANTHROPIC_API_KEY).",
        )
    ...
    try:
        ...
        reply = "\n".join(text_blocks).strip() or "Sorry, I couldn't generate a response."
